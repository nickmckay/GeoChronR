...
We first remotely load the data using the LiPD utilities. This will take a minute as the file is fairly large (2.6Mb).
...
In the first case, we could use the *Lomb-Scargle periodogram* or rather its version tailored for paleoclimate data, [REDFIT](https://www.nonlin-processes-geophys.net/16/43/2009/).
...
Where it is clearly seen that the data appear to contain significant enerngy (peaks) near, but not exactly at, the Milankovitch periodicities (100, 41, 23, and 19 kyr).  These periodicities (particularly the eccentricity (100kyr) and obliquity (40kyr)) rise above the various power law nulls, but we see hints of higher power at high-frequencies. We shall soon see that this is an artifact of Lomb-Scargle. Other methods smooth out that noise.   
...
Also notable is that the confidence limits affect peak detection very differently here; only the eccentricity cycle near 100k rises above the 95% and 99% levels; other cycles are summarily dismissed. This could be a result of assuming `dof = 2` at each frequency, which is most likely violated with wavelets. Internally, `nuwavelet_psd` does use variable DOFs, but this is not easy to export. You can tweak the range over which this power law is fit and it will not change much.

Readers interested in a more robust implementation of WWZ may want to consider a [Python](http://linkedearth.github.io/Pyleoclim_util/Spectral.html) implementation, available from the [Pyleoclim](http://linkedearth.github.io/Pyleoclim_util/Introduction.html) package.  Note that this can be done in R via the [Reticulate](https://rstudio.github.io/reticulate/) package. 

## Time-uncertain spectral analysis 
Now let's consider age uncertainties. 

The GeoChronR approach to quantifying and propagating those uncertainties is to leverage the power of ensembles. Here we will illustrate this with MTM.  Let us repeat the previous analysis by looping over the 1,000 age realizations output by the tuning algorithm HMM-Match. That means computing 1000 spectra.  

```{r ensemble-mode MTM, compute}
time = age$values[age.median < 1000,]  # age ensemble for ~ last 1 Ma
values = temp$values[age.median < 1000]
mtm.ens.spec = computeSpectraEns(time,values,max.ens = 1000, method = 'mtm',tbw=tbw, padfac=5)
```
...
p.mtm.ens <- plotSpectraEns(spec.ens = mtm.ens.spec, cl.df = cl.df, xlims = c(10,150), ylims = c(1e-5,1e-1),
...
# label periodicities of interest
p.mtm.ens <- PeriodAnnotate(p.mtm.ens, periods = c(19,23,41,100), ylims =c(0.01,10))
show(p.mtm.ens)
```

BUG: Period Annotate does not appear here. 

Consistent with previous investigations (e.g. [Mudelsee et al 2009](https://npg.copernicus.org/articles/16/43/2009/)), the effect of age uncertainties is more felt at high frequencies, with the effect of broadening peaks, or lumping them together. The only two peaks that rise above the power-law background are the ~100kyr and ~40kyr peaks, which is not surprising. No doubt a different core with higher-resolution would show a sharper precessonal peak (perhaps even two). 


```{r ensemble-mode REDFIT}
spec.redfit <- computeSpectraEns(time,values,max.ens = 100, method = 'redfit')
redfit.ens.df <- data.frame("freq" = spec.redfit$freq, "pwr" = spec.redfit$power)
cl.ens.df <- data.frame("freq" = spec.redfit$freq, "95% CL" = spec.redfit$power.CL)
names(cl.ens.df)[2] <- "95% CL"
```

again, it took a bit of time. let's compare the 

```{r ensemble-mode REDFIT plot}
p.redfit.ens <- plotSpectraEns(spec.ens = redfit.ens.df, cl.df = cl.ens.df, 
                               xlims = c(10,150), ylims = c(1e-5,1e-1),
                               color.line='darkorange',
                               color.low='darkorange2',
                               color.high='coral4',color.cl = 'white') + 
  ggtitle("IODP 846 age ensemble, MTM") + theme_hc(style = "darkunica") + 
...
# label periodicities of interest
p.redfit.ens <- PeriodAnnotate(p.redfit.ens, periods = c(19,23,41,100), ylims =c(0.01,10))
show(p.mtm.ens)
```
...
# Conclusion
To conclude, we must emphasize that hunting for astronomical (or other) frequencies in paleoclimate timeseries takes extreme care, and cannot be answered in one function call. In this particular example where the data are nearly evenly spaced, and interpolation has a negligible impact, the best choice is MTM for two reasons: (1) because its minimizes spectral leakage, and (2) because the implementation used here (derived from *astrochron*) allows to flexibly apply different tests of significance. Not such luxury is afforded by the other methods  at present, and _nuspectral_ is desperately slow on this dataset. 
