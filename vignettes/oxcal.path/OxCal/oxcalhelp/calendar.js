/*
       JavaScript functions for the Fourmilab Calendar Converter
              ...
              http://www.fourmilab.ch/documents/calendar/
                ...
                Minor modifications by Christopher Bronk Ramsey - June 2006
*/
...
function jd_to_gregorian(jd) {
    var wjd, depoch, quadricent, dqc, cent, dcent, quad, dquad,
        yindex, dyindex, year, yearday, leapadj, day, month;
...
function updateFromGregorian()
{
    var j, year, mon, mday, hour, min, sec,
        weekday, julcal, hebcal, perscal, islcal, hmindex, utime, isoweek,
    ...
    // Added for OxCal
    var bc = document.gregorian.bc.checked;
    var astro = document.gregorian.astro.checked;
    if(astro)
    {
     if(bc)
     {
      alert("Astronomical convention does not use BC/BCE");
      year=-year;
      document.gregorian.year.value=year;
      document.gregorian.bc.checked=false;
     };
    }
    else
    {
     if(year < 1)
     {
      alert("Year 0, or negative not allowed - use BC/BCE or astronomical convention");
      year=-year;
      document.gregorian.year.value=year;
      if(year==0){year=1;};
     };
    ...
    // added for OxCal
    document.juliancalendar.bc.checked=false;
    if(julcal[0]<1)
    {
     julcal[0]=-julcal[0];
     document.juliancalendar.bc.checked=true;
    };
...
//        hmindex + ".gif";
    ...
    // Section added for Oxcal by Christopher Ramsey
    
    // Update OxCal
    
    document.oxcal.year.value = 1+truncYearPrecision((j-GREGORIAN_EPOCH)/(GREGORIAN_YEAR));

    // Update BP
    
    document.bp.year.value = truncYearPrecision(1949.5 - (j-GREGORIAN_EPOCH)/(GREGORIAN_YEAR));

	// Update Besselian
	
    document.besselian.year.value = 2000+truncYearPrecision((j-2451544.53)/(365.242189));

	// Update Julian fractional years
	
	document.julianyear.year.value = 2000+truncYearPrecision((j-J2000)/(JulianCentury/100));
...
function calcJulian()
{
    ...
    // added for OxCal
    document.gregorian.bc.checked=false;
    if(!document.gregorian.astro.checked)
    {
     if(date[0]<1)
     {
      date[0]=1-date[0];
      document.gregorian.bc.checked=true;
     };
    };
...
{
    // Added for OxCal
    var year = new Number(document.juliancalendar.year.value);
    var month = document.juliancalendar.month.selectedIndex + 1;
    var day = new Number(document.juliancalendar.day.value);
    var bc = document.juliancalendar.bc.checked;
    if(bc){year = 1-year;};
    setJulian(julian_to_jd(year,month,day));
}
...
// OxCal addition by Christopher Ramsey - calcOxCal - update from OxCal native format
 
function calcOxCal()
{
    var year = new Number(document.oxcal.year.value);

    setJulian(GREGORIAN_EPOCH+(year-1)*GREGORIAN_YEAR);
}

// OxCal addition by Christopher Ramsey - calcBesselian - update from OxCal native format
 
function calcBesselian()
{
    var year = new Number(document.besselian.year.value);

    setJulian(2451544.53+(year-2000)*365.242189);
}
// OxCal addition by Christopher Ramsey - calcJulianYear - update from OxCal native format
 
function calcJulianYear()
{
    var year = new Number(document.julianyear.year.value);

    setJulian(J2000+(year-2000)*(JulianCentury/100));
}
// OxCal addition by Christopher Ramsey - calcBp - update from OxCal native format
 
function calcBp()
{
    var year = new Number(document.bp.year.value);

    setJulian(GREGORIAN_EPOCH+(1949.5-year)*GREGORIAN_YEAR);
}

// OxCal addition by Christopher Ramsey - truncated precision to 0.3s

function truncYearPrecision(yr)
{
 return Math.round(yr*100000000)/100000000;
};

function getCurrentOxCal()
{
    var today = new Date();
    var y = today.getUTCFullYear();
    var j = gregorian_to_jd(y, today.getUTCMonth() + 1, today.getUTCDate()) +
           (Math.floor(today.getUTCSeconds() + 60 * (today.getUTCMinutes() + 60 * today.getUTCHours()) + 0.5) / 86400.0);
    document.oxcalClock.year.value=1+truncYearPrecision((j-GREGORIAN_EPOCH)/(GREGORIAN_YEAR));
};

function calcOxCalNow()
{
    var year = new Number(document.oxcalClock.year.value);
