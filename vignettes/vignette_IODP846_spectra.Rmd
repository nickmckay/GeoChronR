...
library(ggthemes)
geoChronRPlotTheme = theme_hc(style = "darkunica")
```
...
library(ggplot2)
library(reshape2)
ggplot(df) + geom_line(aes(x=t/1000,y=d18O),colour="orange") + ggtitle("IODP 846 aligment to ProbStack, median age model") + ylab("d18O") + scale_x_continuous(breaks=seq(0,5)) + xlab("Age (Ma)") + scale_y_reverse() + geoChronRPlotTheme
```
```{r}
plotTimeseriesEnsRibbons(ggplot(),X=age,Y=temp,colorLow="orchid",colorHigh="darkorange3",lineColor="orange",lineWidth=0.5,alp=0.3) + theme_hc(style = "darkunica") + scale_x_reverse()
```
...
Sure enough, this gets rid of a few cycles, but many remain. 
...
nuspec <-  nuspectral:::nuwavelet_psd(t,X,sigma=0.02,taus = tau)
