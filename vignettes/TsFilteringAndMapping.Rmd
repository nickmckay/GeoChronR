...
iso <- readLipd("http://lipdverse.org/iso2k/0_15_0/iso2k0_15_0.zip")
```
...
varName <- pullTsVariable(iTS,"paleoData_variableName")

index <- which(between(lat,10,50) & between (lon,70,170) & varName == "d18O")
...
```{r}
plotTimeAvailabilityTs(gTS,ageRange = c(1,2000),ageVar = "year")
```
...
```{r}
summaryPlotTs(gTS,ageVar = "year", ageRange = c(0,2000))
```
