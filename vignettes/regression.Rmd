...
```{r,echo = FALSE}
knitr::opts_chunk$set(fig.width = 8, fig.height = 6, dev = "ragg_png")
...
## Calculate sedrate
Now that we have an age model, we can also take a look at what the age model implies about sedimentation rates, and their uncertainties. So let's calculate an ensemble of sedimentation rates using the members in the age ensemble. The `createSedRateEnsemble()` function will do this, and create a new paleoData model ensemble table. 

```{r}
#create the table
K <- createSedRateEnsemble(K) #you may need to modify some parameters to this function if you have a complex LiPD object

#now let's see how it looks. We can pull sed rates and the corresponding ages out of that table
srEns <- selectData(K,table.type = "ensemble",var.name = "sedimentationRate")
srAgeEns <- selectData(K,table.type = "ensemble",var.name = "ageEnsemble")

#and plot it
plotTimeseriesEnsRibbons(X = srAgeEns, Y = srEns,probs = c(0.25,0.5,0.75), x.bin = seq(-70,5500,by = 20))
```
There's quite a bit of uncertainty here. This is because we're looking at both the uncertainty in age, and the uncertainty in sedimentation rate, which is the first derivative of age. Often, it's useful to estimate flux, which is the product of sedimentation rate, density and concentration (for mass based concentrations), or sedimentation rate and concentration (for volumetric concentrations). Let's calculate chla flux. Spectrally-inferred chla is representative of volumetric concentration (g/cm^3), so we don't need density for this. 

We already selected our sed rate ensembles, so now let's get our concentration proxy (RABD)

```{r}

  concEns <- selectData(K,table.type = "meas",var.name = "RABD")
  concAgeEns <- selectData(K,table.type = "meas",var.name = "ageEnsemble")
  

K <- calculateFlux(K, 
                   sed.rate.ens = srEns, 
                   sed.rate.age = srAgeEns,
                   density.var = NA,
                   density.var.age = NA,
                   concentration.var = concEns,
                   concentration.var.age = concAgeEns,
                   bin.step = 50)
```
Let's take a look at this estimate of chloropigment flux:

```{r}

#now let's see how it looks. We can pull sed rates and the corresponding ages out of that table
chlFlux <- selectData(K,model.num = 2,table.type = "ensemble",var.name = "flux")
chlFluxAge <- selectData(K,model.num = 2,table.type = "ensemble",var.name = "age")

#and plot it
plotTimeseriesEnsRibbons(X = chlFluxAge, Y = chlFlux,probs = c(0.25,0.5,0.75), x.bin = chlFluxAge$values)
```
Interesting. It looks a lot like sed rate, as fluxes often do. For now, we'll stick with concentration.

## Now load in the instrumental data we want to correlate and regress against
```{r}
kurupa.instrumental <- readr::read_csv(system.file("extdata", "KurupaInstrumental.csv", package = "geoChronR"))
```
